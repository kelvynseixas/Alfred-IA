...
const App = () => {
  ...
  const handleAIAction = async (action: { type: AIActionType, payload: any }) => {
      ...
      try {
          if (action.type === 'ADD_TRANSACTION') {
              await fetch('/api/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(action.payload) });
              fetchDashboardData();
          } 
          // ... (outros handlers IA simplificados) ...
          else if (action.type === 'ADD_TASK') {
               await fetch('/api/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({...action.payload, status: TaskStatus.PENDING}) });
               fetchDashboardData();
          }
  ...
  const getDueItems = () => {
      ...
      return tasks.filter(t => t.status !== TaskStatus.DONE && t.date === today);
  };
    ...
    <>
        <div>
          ...
          <nav className="space-y-1">
            ...
            {currentUser?.role === 'ADMIN' && <button onClick={() => { setActiveModule(ModuleType.ADMIN); setIsMobileMenuOpen(false); }} className={btnClass(ModuleType.ADMIN)}><Settings size={20}/> Painel Master</button>}
          </nav>
        </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        <header className="h-16 border-b border-slate-800 bg-slate-900/95 backdrop-blur flex items-center justify-between px-4 md:px-8 z-40 shrink-0">
             <div className="flex items-center gap-4">
                ...
                <div className="md:hidden flex items-center gap-2"><div className="w-8 h-8 rounded-full overflow-hidden border border-gold-500"><img src={ALFRED_ICON_URL} alt="Alfred" className="w-full h-full object-cover" /></div><span className="font-serif font-bold text-white">Alfred</span></div>
             </div>
             <div className="flex items-center gap-4">
                <button onClick={() => { setShowNotifications(!showNotifications); setShowAnnouncements(false); }} className="p-2 rounded-full bg-slate-800 text-slate-300 hover:text-white relative"><Bell size={20} />{unreadCount > 0 && <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border-2 border-slate-900"></span>}</button>
             </div>
        </header>

        <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
            {activeModule === ModuleType.FINANCE && <FinancialModule 
                transactions={transactions} 
                accounts={accounts}
                onAddTransaction={async (t) => { await fetch('/api/transactions', {method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${localStorage.getItem('alfred_token')}`},body:JSON.stringify(t)}); fetchDashboardData(); }} 
                onEditTransaction={async (id, u) => { await fetch(`/api/transactions/${id}`, {method:'PATCH',headers:{'Content-Type':'application/json','Authorization':`Bearer ${localStorage.getItem('alfred_token')}`},body:JSON.stringify(u)}); fetchDashboardData(); }} 
                onDeleteTransaction={async (id) => { await fetch(`/api/transactions/${id}`, {method:'DELETE',headers:{'Authorization':`Bearer ${localStorage.getItem('alfred_token')}`}}); fetchDashboardData(); }} 
                onAddAccount={async (a) => { await fetch('/api/accounts', {method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${localStorage.getItem('alfred_token')}`},body:JSON.stringify(a)}); fetchDashboardData(); }}
        ...
        <button onClick={() => setIsChatOpen(true)} className="fixed bottom-8 right-8 w-14 h-14 bg-gold-600 hover:bg-gold-500 rounded-full flex items-center justify-center shadow-2xl hover:scale-105 transition-transform z-50">
          <div className="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center overflow-hidden"><img src={ALFRED_ICON_URL} className="w-full h-full object-cover" /></div>
        </button>
        <AlfredChat appContext={{ tasks, transactions, lists, projects, accounts }} isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} onAIAction={handleAIAction} />
      </main>
    </div>
  );
};
