  ...
  return (
    <div className="space-y-6 animate-fade-in">
        <header className="flex justify-between items-center mb-6">
            <div><h2 className="text-3xl font-serif text-white">Projetos & Reservas</h2><p className="text-slate-400">Metas financeiras e fundos de emergência.</p></div>
            <button onClick={openNewProject} className="bg-gold-600 hover:bg-gold-500 text-slate-950 px-4 py-2 rounded-lg font-bold flex items-center gap-2"><Plus size={18} /> Novo Projeto</button>
        </header>

        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="text-center md:border-r md:border-slate-700 last:border-0"><p className="text-xs uppercase text-slate-500 font-bold mb-1">Total Acumulado</p><p className="text-2xl font-bold text-emerald-400">{formatCurrency(totalSaved)}</p></div>
            <div className="text-center md:border-r md:border-slate-700 last:border-0"><p className="text-xs uppercase text-slate-500 font-bold mb-1">Meta Global</p><p className="text-2xl font-bold text-white">{formatCurrency(totalTarget)}</p></div>
            <div className="text-center"><p className="text-xs uppercase text-slate-500 font-bold mb-1">Progresso Geral</p><p className="text-2xl font-bold text-gold-500">{totalTarget > 0 ? ((totalSaved/totalTarget)*100).toFixed(1) : 0}%</p></div>
        </div>
            ...
            {projects.map(proj => {
                ...
                <div key={proj.id} className="rounded-xl border p-6 flex flex-col bg-slate-800 border-slate-700">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${proj.category === 'RESERVE' ? 'bg-blue-500/10 text-blue-400' : 'bg-gold-500/10 text-gold-400'}`}>{proj.category === 'RESERVE' ? <PiggyBank size={20}/> : <Target size={20}/>}</div>
                            <div><h3 className="font-bold text-white">{proj.title}</h3><p className="text-xs text-slate-500">{proj.deadline ? new Date(proj.deadline).toLocaleDateString() : 'Sem prazo'}</p></div>
                        </div>
                        <button onClick={() => onDeleteProject(proj.id)} className="text-slate-500 hover:text-red-400"><Trash2 size={16}/></button>
                    </div>
                    
                    <div className="mb-4"><div className="flex justify-between text-sm mb-1"><span className="text-slate-400">Progresso</span><span className="text-white font-bold">{progress.toFixed(1)}%</span></div><div className="w-full bg-slate-700 rounded-full h-2"><div className="h-2 rounded-full bg-gold-500" style={{width: `${progress}%`}}></div></div></div>
             ...
             <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-md p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <button type="submit" className="w-full bg-gold-600 text-slate-950 font-bold py-2 rounded">Criar Projeto</button>
                    </form>
                </div>
             </div>
        )}
