...
import { Plus, ArrowUpCircle, ArrowDownCircle, TrendingUp, DollarSign, Repeat, Target, Trash2 } from 'lucide-react';
...
export const FinancialModule: React.FC<FinancialModuleProps> = ({ transactions, accounts, projects, onAddTransaction, onDeleteTransaction, onAddAccount }) => {
  ...
  // Investimentos agora são um módulo separado, o financeiro foca em fluxo de caixa
  const totals = {
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center">
          <h3 className="text-white font-serif">Histórico Recente</h3>
        </div>
          ...
          {transactions.map(t => (
            <div key={t.id} className="p-4 flex justify-between items-center hover:bg-slate-800/50 transition-colors group">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => onDeleteTransaction(t.id)} className="text-slate-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
