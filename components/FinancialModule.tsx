...
export const FinancialModule: React.FC<FinancialModuleProps> = ({ transactions, onAddTransaction, onDeleteTransaction, isDarkMode }) => {
  ...
  const [newTrans, setNewTrans] = useState<{description: string, amount: string, type: TransactionType, category: string, recurrence: 'NONE' | 'MONTHLY' | 'WEEKLY'}>({ 
      description: '', amount: '', type: TransactionType.EXPENSE, category: '', recurrence: 'NONE' 
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setNewTrans({ description: '', amount: '', type: TransactionType.EXPENSE, category: '', recurrence: 'NONE' });
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`p-6 rounded-xl border backdrop-blur-sm group hover:border-emerald-500/30 transition-colors ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'}`}>
             ...
             <button onClick={() => setDetailsView(TransactionType.INCOME)} className={`w-full py-2 text-sm rounded flex items-center justify-center gap-2 transition-colors ${isDarkMode ? 'bg-slate-700/50 hover:bg-emerald-500/10 text-emerald-400' : 'bg-slate-100 hover:bg-emerald-50 text-emerald-600'}`}>
        ...
        <div className={`p-6 rounded-xl border backdrop-blur-sm group hover:border-red-500/30 transition-colors ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'}`}>
             ...
             <button onClick={() => setDetailsView(TransactionType.EXPENSE)} className={`w-full py-2 text-sm rounded flex items-center justify-center gap-2 transition-colors ${isDarkMode ? 'bg-slate-700/50 hover:bg-red-500/10 text-red-400' : 'bg-slate-100 hover:bg-red-50 text-red-600'}`}>
                <List size={14} /> Ver Detalhes
             </button>
        </div>
        <div className={`p-6 rounded-xl border backdrop-blur-sm group hover:border-gold-500/30 transition-colors ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'}`}>
             ...
             <button onClick={() => setDetailsView(TransactionType.INVESTMENT)} className={`w-full py-2 text-sm rounded flex items-center justify-center gap-2 transition-colors ${isDarkMode ? 'bg-slate-700/50 hover:bg-gold-500/10 text-gold-400' : 'bg-slate-100 hover:bg-gold-50 text-gold-600'}`}>
                <List size={14} /> Ver Detalhes
             </button>
        </div>
      </div>
            ...
            <div className={`${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'} border rounded-xl w-full max-w-md p-6 shadow-2xl`}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input className={`w-full border rounded p-2 focus:border-gold-500 focus:outline-none ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'}`} value={newTrans.description} onChange={e => setNewTrans({...newTrans, description: e.target.value})} required />
                    </div>
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Valor (R$)</label>
                        <input type="number" step="0.01" className={`w-full border rounded p-2 focus:border-gold-500 focus:outline-none ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'}`} value={newTrans.amount} onChange={e => setNewTrans({...newTrans, amount: e.target.value})} required />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs text-slate-400 mb-1">Categoria</label>
                            <input className={`w-full border rounded p-2 focus:border-gold-500 focus:outline-none ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'}`} placeholder="Ex: Salário" value={newTrans.category} onChange={e => setNewTrans({...newTrans, category: e.target.value})} />
                        </div>
                         <div>
                            <label className="block text-xs text-slate-400 mb-1">Recorrência</label>
                            <select value={newTrans.recurrence} onChange={e => setNewTrans({...newTrans, recurrence: e.target.value as any})} className={`w-full border rounded p-2 focus:border-gold-500 focus:outline-none ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'}`}>
                                ...
                                {filteredTransactionsForDetails.slice().reverse().map(t => (
                                    <tr key={t.id} className={`border-b transition-colors ${isDarkMode ? 'border-slate-800 hover:bg-slate-800/30' : 'border-slate-100 hover:bg-slate-50'}`}>
                                        ...
                                        <td className="px-4 py-3">
                                            {t.recurrence === 'MONTHLY' && <span className="flex items-center gap-1 text-blue-400"><Repeat size={12} /> Mensal</span>}
                                            {t.recurrence === 'WEEKLY' && <span className="flex items-center gap-1 text-purple-400"><Repeat size={12} /> Semanal</span>}
