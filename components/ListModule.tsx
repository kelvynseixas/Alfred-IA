  ...
  return (
    <div className="space-y-6 animate-fade-in">
        <header className="mb-6 flex justify-between items-end">
          <div><h2 className="text-3xl font-serif text-white">Inventário & Listas</h2><p className="text-slate-400">Garantindo que a propriedade esteja bem abastecida.</p></div>
          <button onClick={openNewListModal} className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg text-sm transition-colors flex items-center gap-2"><Plus className="w-4 h-4" /> Nova Lista</button>
        </header>
            ...
            {lists.map(list => {
                ...
                return (
                    <div key={list.id} className={`bg-slate-800 rounded-xl border transition-all overflow-hidden ${isExpanded ? 'border-gold-500/50 shadow-lg' : 'border-slate-700 hover:border-slate-600'}`}>
                        <div onClick={() => toggleExpand(list.id)} className="p-4 flex justify-between items-center cursor-pointer hover:bg-slate-700/30">
                            <div className="flex items-center gap-4">
                                <div className={`p-2 rounded-full ${list.name.toLowerCase().includes('compra') ? 'bg-gold-500/20 text-gold-500' : 'bg-slate-700 text-slate-400'}`}>{list.name.toLowerCase().includes('compra') ? <ShoppingCart size={20} /> : <CheckSquare size={20} />}</div>
                                <div><h3 className="font-serif text-lg text-white font-medium">{list.name}</h3><p className="text-xs text-slate-400">{pendingCount} itens pendentes</p></div>
                            </div>
                            <div className="flex items-center gap-4">
                        ...
                        {isExpanded && (
                            <div className="p-4 border-t border-slate-700 bg-slate-900/30">
                                    ...
                                    {list.items && list.items.map(item => (
                                        <li key={item.id} className="flex items-center justify-between p-2 rounded hover:bg-slate-700/30">
                                            <div className="flex items-center gap-3">
                                                <button onClick={() => onToggleItem(list.id, item.id)} className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${item.status === ItemStatus.DONE ? 'bg-emerald-500/20 border-emerald-500 text-emerald-500' : 'border-slate-500 hover:border-gold-500'}`}><div className={`w-2.5 h-2.5 bg-current rounded-sm ${item.status === ItemStatus.DONE ? 'opacity-100' : 'opacity-0'}`} /></button>
                                ...
                                <div className="flex gap-2">
                                    <input type="text" placeholder="Novo item..." value={newItemName[list.id] || ''} onChange={(e) => setNewItemName({...newItemName, [list.id]: e.target.value})} onKeyDown={(e) => e.key === 'Enter' && handleAddItem(list.id)} className="flex-1 bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-white focus:outline-none focus:border-gold-500" />
                                    <button onClick={() => handleAddItem(list.id)} disabled={loadingItem === list.id} className="bg-gold-600 hover:bg-gold-500 text-slate-950 px-4 rounded transition-colors disabled:opacity-50 font-bold">{loadingItem === list.id ? <Loader2 className="animate-spin" size={16}/> : <Plus size={16}/>}</button>
                                </div>
                            </div>
                        )}
                    </div>
             ...
             <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-sm p-6 shadow-2xl">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={handleSubmitList} className="flex-1 bg-gold-600 hover:bg-gold-500 text-slate-950 font-bold py-2 rounded">Criar</button>
                    </div>
                </div>
             </div>
        )}
