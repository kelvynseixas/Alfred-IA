           ...
           <div className="space-y-6">
               <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                       ...
                       <button onClick={handleCreateUser} disabled={loading} className="bg-emerald-600 hover:bg-emerald-500 text-white p-2 rounded text-sm font-bold">{loading ? <Loader2 className="animate-spin mx-auto"/> : 'Criar'}</button>
                   </div>
               </div>
                   ...
                   <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
                       <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-md">
                           ...
                           <div className="space-y-4">
                               ...
                               <div><label className="text-xs text-slate-400">Adicionar Dias</label><div className="flex gap-2"><button onClick={() => setUserEditForm({...userEditForm, trialDaysToAdd: 30})} className="bg-slate-800 text-white px-3 py-1 rounded text-xs">+30d</button></div></div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                   {coupons?.map(c => (
                       <div key={c.id} className="bg-slate-800 border border-slate-700 p-4 rounded-lg flex justify-between items-center">
                           <div><p className="text-white font-bold">{c.code}</p><p className="text-xs text-slate-400">{c.type === 'PERCENTAGE' ? `${c.value}%` : `R$ ${c.value}`}</p></div>
